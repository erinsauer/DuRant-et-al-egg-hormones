...
############# function for figures 2 & 3 ########################
make_halfviolin_mean_ci <- function(disagg, emm, whichpart_value, title_txt,
                                    show_legend = FALSE) {
  
  disagg <- disagg %>%
    dplyr::filter(whichpart == whichpart_value)
  
  ggplot(disagg, aes(x = MGD, y = response, fill = Diet, color = Diet)) +
    ggdist::stat_halfeye(adjust = 0.6, width = 0.6, .width = 0,
                         justification = -0.2, point_color = NA,
                         slab_color = NA, alpha = 0.5) +
    geom_point(data = emm, aes(y = emmean), position = position_nudge(x = 0.15),
               size = 2.8, show.legend = FALSE) +
    geom_errorbar(data = emm, aes(y=emmean, ymin = lower.CL, ymax = upper.CL),
                  width = 0.08, position = position_nudge(x = 0.15),
...
PC1 <- make_halfviolin_mean_ci(
  disagg = PC_disaggregated,
  emm    = interaction.table.PC1,
  whichpart_value = "1",
  title_txt = "PC1",
  show_legend = TRUE
)

PC2 <- make_halfviolin_mean_ci(
  disagg = PC_disaggregated,
  emm    = interaction.table.PC2,
  whichpart_value = "2",
  title_txt = "PC2",
  show_legend = FALSE
)

PC3 <- make_halfviolin_mean_ci(
  disagg = PC_disaggregated,
  emm    = interaction.table.PC3,
  whichpart_value = "3",
  title_txt = "PC3",
  show_legend = FALSE
)
...
andro.fig <- make_halfviolin_mean_ci(
  disagg = andro_disaggregated,
  emm    = andro.table.andro,
  whichpart_value = "1",
  title_txt = "Androstenedione",
  show_legend = TRUE
)

testo.fig <- make_halfviolin_mean_ci(
  disagg = andro_disaggregated,
  emm    = andro.table.testo,
  whichpart_value = "2",
  title_txt = "Testosterone",
  show_legend = FALSE
)

etio.fig <- make_halfviolin_mean_ci(
  disagg = andro_disaggregated,
  emm    = andro.table.ei,
  whichpart_value = "3",
  title_txt = "Etiocholanolone",
  show_legend = FALSE
)
...
proges.fig <- make_halfviolin_mean_ci(
  disagg = prog_disaggregated,
  emm    = prog.table.proges,
  whichpart_value = "1",
  title_txt = "Progesterone",
  show_legend = FALSE
)

pregne.fig <- make_halfviolin_mean_ci(
  disagg = prog_disaggregated,
  emm    = prog.table.pregne,
  whichpart_value = "2",
  title_txt = "Pregnenolone",
  show_legend = FALSE
)

hp.fig <- make_halfviolin_mean_ci(
  disagg = prog_disaggregated,
  emm    = prog.table.hp,
  whichpart_value = "3",
  title_txt = "17α-hydroxypregnenolone",
  show_legend = FALSE
)

dhp.fig <- make_halfviolin_mean_ci(
  disagg = prog_disaggregated,
  emm    = prog.table.dhp,
  whichpart_value = "4",
  title_txt = "Pregnanedione",
  show_legend = FALSE
)

pregna.fig <- make_halfviolin_mean_ci(
  disagg = prog_disaggregated,
  emm    = prog.table.pregna,
  whichpart_value = "5",
  title_txt = "Pregnanolone",
  show_legend = FALSE
)
