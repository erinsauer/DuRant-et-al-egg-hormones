...
part1 <- cbind(hormPC, response = (hormPC$PC1), whichpart = factor(1))
part2 <- cbind(hormPC, response = (hormPC$PC2), whichpart = factor(2))
part3 <- cbind(hormPC, response = (hormPC$PC3), whichpart = factor(3))
...
andro.model.MG <- lme(response ~ whichpart - 1 + 
                        whichpart:(Yolk.Mass+Fat.y+load.AUC*Diet+Egg.),  
                      random = ~ whichpart -1 |Bird.ID, 
                      weights=varIdent(form=~1|whichpart),
                      corr=corSymm(form=~1|Bird.ID/Tube.number),
...
###### FIgures for Sarah - SICB ############
andro.fig <- subset(andro_disaggregated, whichpart==1)
andro.fig <- subset(andro.fig, Treatment=="MG")
colnames(andro.fig)

ggplot(andro.fig, aes(x=load.AUC, y=response, color=Diet)) +
  geom_smooth(method=lm) +
  theme_bw()+
  theme(axis.text=element_text(size=10, color="black"),
        panel.border = element_blank(),
        ...
        legend.title=element_blank(),
        plot.title=element_text(size=30, hjust=0.5))+
  ylab("Scaled concentration in eggs")+xlab("MG load AUC")+ggtitle("Androstenedione")

prog.fig <- subset(prog_disaggregated, whichpart==1)
prog.fig <- subset(prog.fig, Treatment=="MG")
colnames(prog.fig)

ggplot(prog.fig, aes(x=load.AUC, y=response, color=Diet)) +
  geom_smooth(method=lm) +
  theme_bw()+
  theme(axis.text=element_text(size=10, color="black"),
        panel.border = element_blank(),
        ...
        legend.title=element_blank(),
        plot.title=element_text(size=30, hjust=0.5))+
  ylab("Scaled concentration in eggs")+xlab("MG load AUC")+ggtitle("Progesterone")
...
prog.model <- lme(response ~ whichpart - 1 + 
                    whichpart:(Diet*Treatment+Egg.+Mass.y+Yolk.Mass),  
                  random = ~ whichpart -1 |Bird.ID, 
                  #weights=varIdent(form=~1|whichpart),
                  corr=corSymm(form=~1|Bird.ID/Tube.number),
...
mass.fig <- ggplot(prog_disaggregated, aes(x=Mass.y, y=response, color=whichpart)) +
  #geom_point() +
  geom_smooth(method="lm")+
  theme_bw()+
  theme(axis.text=element_text(size=10, color="black"),
  ...
  ylab("Scaled concentration")+xlab("Maternal mass (g)")+ggtitle("Progestagens")
mass.fig
...
prog.model.MG <- lme(response ~ whichpart - 1 +
                       whichpart:(Yolk.Mass+Diet*load.AUC+Mass.y) ,  
                     random = ~ whichpart -1 |Bird.ID, 
                     weights=varIdent(form=~1|whichpart),
                     corr=corSymm(form=~1|Bird.ID/Tube.number),
...
colnames(moms) 
colnames(horm2)
moms3 <- (moms[,c(7,8,12,13,17)])
horm4 <- merge(moms3, horm2, by="Bird.ID")

massm1 <- lmer(Yolk.Mass ~ Diet * Treatment + 
                 csize + (1|Bird.ID), data=horm4)
summary(massm1)
Anova(massm1)
#save tables
...
str(moms)
colnames(horm2)
hormsub <- (horm2[,c(2,4,5)])
horm4 <- merge(moms3, horm2, by="Bird.ID")

#clutch size model
csizem <- glm(csize ~ Diet * Treatment, data=moms)
summary(csizem)
Anova(csizem)
#save tables
...
#total clutch mass
tmassm <- glm(sum.egg.mass ~ Diet * Treatment, data=moms)
summary(tmassm)
Anova(tmassm)
#save tables
